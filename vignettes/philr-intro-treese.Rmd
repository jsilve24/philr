---
title: "Introduction to PhILR: TreeSE / miaverse"
author: "Leo Lahti and Justin Silverman"
date: "`r Sys.Date()`"
bibliography: philr.bib
output: BiocStyle::html_document
vignette: >
  %\VignetteIndexEntry{Introduction to PhILR: TreeSE}
...
data format in this vignette [@huang2021]. Examples for the same
workflow is available also for the alternative
[phyloseq](philr-intro-phyloseq.md) format.

This page shows how to prepare `TreeSE` data object for philr
analysis. We show the GlobalPatterns example workflow as initially
outlined in [@mcmurdie2013].

The TreeSE version for the GlobalPatterns data is provided with the
[`mia` package](microbiome.github.io/mia) [@lahti2020].
...
GP_tse <- GlobalPatterns # Rename to make explicit that this data is in TreeSE format
```
...
tse <-  GP_tse %>% subsetByPrevalentTaxa(
                               detection = 3,
                               prevalence = 20/100,
                               as_relative = FALSE)
...
Here we will use the same weightings as we used in the main paper. You
can provide the data directly in TreeSE format.
...
```{r}
# Calculate PCoA (MDS)
gp.pcoa <- ecodist::pco(gp.dist)
d <- as.data.frame(gp.pcoa$vectors)
names(d) <- paste0("PC", 1:ncol(d))

# Add metadata
d$SampleType <- factor(colData(tse)$SampleType)
...
# TODO: add this later once runMDS issues have been sorted out
# An alternative visualization 
# tse <- makeTreeSummarizedExperimentFromphyloseq(pseq)
# Add the phILR transformed data as altExp (it has n-1 features)
# se <- SummarizedExperiment(t(gp.philr));
# rowData(se)$feature <- colnames(gp.philr);
# altExp(tse, "phILR") <- se
# # Plot with runMDS and plotReducedDim
# se <- runMDS(tse, FUN = dist, name = "MDS_phILR", altexp="phILR")
