...
The goal of PhILR is to transform input microbiota data into a Euclidean vector space while preserving all information contained in the original composition.  For a given set of  samples consisting of measurements of  taxa, we transform data into a new space of  samples and  orthonormal coordinates termed ‘balances’ (Fig. 1). Each balance  is associated with a single internal node  of a phylogenetic tree with the taxa as leaves. The balance represents the log-ratio of the geometric means of the two groups of taxa that descend from (TODO: Citation to Main Paper). 
...
Taxa that were not seen with more than 3 counts in at aleast 20% of samples are removed
and taxa with a coefficient of variation ≤ 3.0 are removed. This later step is 
intended to "protect against OTUs with small mean & trivially large C.V."
[@mcmurdie2013]. Finally we add a pseudocount of 1 to the remaining OTUs to avoid 
calculating log-ratios involving zeros. Note that downstream weighting procedures
make the choice of this pseudocount relatively unimportant(TODO: Cite our paper). 
Alternatively other replacement methods (multiplicative replacement etc...) may be 
...
`philr::buildilrBasep()`
4. Convert OTU table to relative abundance and 'shift' dataset using the weightings
[@egozcue2016] using the function `philr::shiftp()`.
...
Here we will use the same weightings as we used in the main paper (TODO: Cite main paper).

```{r}
gp.philr <- philr(otu.table, tree, 
                  part.weights='anorm.x.gm.counts', 
...
So these are the top 5 coordinates (balances) that distinguish whether a 
sample is from a human or non-human source. Now using the `ggtree` package we 
can visualize these balances on the tree. To use this function we need
to know the acctual node number (not just the names we have given) of these balances
on the tree. To convert between node number and name, we have written the functions
`philr:` TODO: Add these in and export functions. 

```{r}
ggtree(tree) + 
  geom_balance(aes(node=(label %in% top.coords), fill=label),
               color='white',
               alpha=0.6, 
               extend=1)

ggtree(tree) + 
  geom_balance(node='n16', 
               color='white',
               alpha=0.6, 
               extend=1)
```
