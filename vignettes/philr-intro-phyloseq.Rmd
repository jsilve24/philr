---
title: "Introduction to PhILR: phyloseq"
author: "Leo Lahti and Justin Silverman"
date: "`r Sys.Date()`"
bibliography: philr.bib
output: BiocStyle::html_document
vignette: >
  %\VignetteIndexEntry{Introduction to PhILR: phyloseq}
...
Here we will demonstrate PhILR analysis using the Global Patterns
...
# Data preparation (phyloseq)

We retrieve the example data in [phyloseq](data_phyloseq.md) data
format in this vignette. Examples for the same data preparation
procedure is available also for the alternative
[TreeSummarizedExperiment](data_TreeSE.md) format.

The GlobalPatterns dataset is provided with the `phyloseq` package
[@mcmurdie2013] and our analysis follows, in part, that of the authors
[GitHub Tutorial](http://joey711.github.io/phyloseq/preprocess.html).

```{r, echo=TRUE, message=FALSE}
library(phyloseq); packageVersion("phyloseq")
data(GlobalPatterns, package = "phyloseq")
GP_pseq <- GlobalPatterns # Rename to make explicit that this data is in phyloseq format
```
...
variation ≤ 3 are filtered. These steps follow those of
[@mcmurdie2013]. Finally we add a pseudocount of 1 to the remaining
OTUs to avoid calculating log-ratios involving zeros. Alternatively
other replacement methods (multiplicative replacement etc...) may be
used instead if desired; the subsequent taxa weighting procedure we
will describe complements a variety of zero replacement methods.

```{r, message=FALSE, warning=FALSE}
pseq <- filter_taxa(GP_pseq, function(x) sum(x > 3) > (0.2*length(x)), TRUE)
pseq <- filter_taxa(pseq, function(x) sd(x)/mean(x) > 3.0, TRUE)
pseq <- transform_sample_counts(pseq, function(x) x+1)
```
...
# Add the modified tree back to the (phyloseq) data object 
phy_tree(pseq) <- tree
```
...
```{r}
name.balance(phy_tree(pseq), tax_table(pseq), 'n1')
```
...
```{r, message=FALSE, warning=FALSE}
human.samples <- factor(get_variable(pseq, "SampleType") %in% c("Feces", "Mock", "Skin", "Tongue"))
```
...
Alternatively, you can provide the data directly in phyloseq format.
...
each balance is associated with a single internal node of the tree, we
denote the balances using the same names we assigned to the internal
nodes (e.g., `n1`).
...
```{r}
gp.pcoa <- ordinate(pseq, 'PCoA', distance=gp.dist)
plot_ordination(pseq, gp.pcoa, color='SampleType') + geom_point(size=4)
```
...
logistic regression (from the `glmnet` package) to identify a small number of 
...
We can also get more information on what goes into the naming by
viewing the votes directly.
...
human/non-human sources.  In order to plot with `ggplot2` we first
need to convert the PhILR transformed data to long format. We have
included a function `philr::convert_to_long()` for this purpose.
