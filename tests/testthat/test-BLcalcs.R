...
tr.fixed <- ape::read.tree(text="((t2:0.3708901014,((t1:0.5037344745,
                           t3:0.5299885271)n4:0.7873590044,
                           t5:0.4477706181)n3:0.8255127743)n2:0.1799265754,
                           t4:0.9563850679)n1;")

test_that('mean.dist.to.tips gives correct results on fixed tree', {
  mdtt <- mean.dist.to.tips(tr.fixed)
...
test_that('calculate.blw handles tip with edge length zero and gives correct results', {
  tr <- ape::read.tree(text="((t1:0.3187429151,t2:0):0.3409723381,
                       t3:0.04305356415);")
